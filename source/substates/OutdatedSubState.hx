...
class OutdatedSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		warnText = new FlxText(0, 0, FlxG.width,
			'Sup bro, looks like you\'re running an outdated version of\nPsych Engine with Python Support\nvia hxpy (${MainMenuState.engineVersion})\n
			-----------------------------------------------\n
	...
	override function update(elapsed:Float)
	{
		if(!leftState) {
			if (controls.ACCEPT) {
				leftState = true;
				CoolUtil.browserLoad("https://github.com/Mark-bitde/FNF-PsychEngine-WithPython-hxpy/releases");
			}
